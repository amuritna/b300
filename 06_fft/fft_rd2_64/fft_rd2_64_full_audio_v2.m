...
% new in v2: separation of imaginary/real parts
...
hold off;
title('manual FFT');
title(['manual FFT - diff = ', num2str(sum(abs(f_bi-(f_man_re + f_man_im*1i)))), ', (mean ', num2str(sum(abs(f_bi-(f_man_re + f_man_im*1i)))/floor(length(yt)/N)), ')']);

% function declaration
function [y_re, y_im] = fft64(xn)
N = 64;
...
for mm = 0:1:31
    ...
    twiddle_re = cos(-2 * pi * mm * 1/64);
    twiddle_im = sin(-2 * pi * mm * 1/64);

    x1_re(mm+1) = xn(mm+1) + xn(mm+33);
    % no x1_im yet
    x1_re(mm+33) = (xn(mm+1) - xn(mm+33)) * twiddle_re;
    x1_im(mm+33) = (xn(mm+1) - xn(mm+33)) * twiddle_im;
end
x1 = x1_re + 1i*x1_im;
...
for mm = 0:1:15
    ...
    twiddle_re = cos(-2 * pi * mm * 1/32);
    twiddle_im = sin(-2 * pi * mm * 1/32);

    for shiftB = (N/4+1):(N/4):(N*3/4+1)
        shiftA = shiftB - (N/4);
        x2_re(mm+shiftA) = x1_re(mm+shiftA) + x1_re(mm+shiftB);
        x2_im(mm+shiftA) = x1_im(mm+shiftA) + x1_im(mm+shiftB);
        x2_re(mm+shiftB) = (x1_re(mm+shiftA) - x1_re(mm+shiftB)) * twiddle_re;
        x2_im(mm+shiftB) = (x1_im(mm+shiftA) - x1_im(mm+shiftB)) * twiddle_im;
    end
end
x2 = x2_re + 1i*x2_im;
...
for mm = 0:1:7
    ...
    twiddle_re = cos(-2 * pi * mm * 1/16);
    twiddle_im = sin(-2 * pi * mm * 1/16);

    for shiftB = (N/8+1):(N/8):(N*7/8+1)
        shiftA = shiftB - (N/8);
        x3_re(mm+shiftA) = x2_re(mm+shiftA) + x2_re(mm+shiftB);
        x3_im(mm+shiftA) = x2_im(mm+shiftA) + x2_im(mm+shiftB);
        x3_re(mm+shiftB) = (x2_re(mm+shiftA) - x2_re(mm+shiftB)) * twiddle_re;
        x3_im(mm+shiftB) = (x2_im(mm+shiftA) - x2_im(mm+shiftB)) * twiddle_im;
    end
end
...
for mm = 0:1:3
    ...
    twiddle_re = cos(-2 * pi * mm * 1/8);
    twiddle_im = sin(-2 * pi * mm * 1/8);
    
    for shiftB = (N/16+1):(N/16):(N*15/16+1)
        shiftA = shiftB - (N/16);
        x4_re(mm+shiftA) = x3_re(mm+shiftA) + x3_re(mm+shiftB);
        x4_im(mm+shiftA) = x3_im(mm+shiftA) + x3_im(mm+shiftB);
        x4_re(mm+shiftB) = (x3_re(mm+shiftA) - x3_re(mm+shiftB)) * twiddle_re;
        x4_im(mm+shiftB) = (x3_im(mm+shiftA) - x3_im(mm+shiftB)) * twiddle_im;
    end
end

% stage 5 -> N = 64, M = 4
for mm = 0:1:1
    twiddle_re = cos(-2 * pi * mm * 1/4);
    twiddle_im = sin(-2 * pi * mm * 1/4);
    
    for shiftB = (N/32+1):(N/32):(N*31/32+1)
        shiftA = shiftB - (N/32);
        x5_re(mm+shiftA) = x4_re(mm+shiftA) + x4_re(mm+shiftB);
        x5_im(mm+shiftA) = x4_im(mm+shiftA) + x4_im(mm+shiftB);
        x5_re(mm+shiftB) = (x4_re(mm+shiftA) - x4_re(mm+shiftB)) * twiddle_re;
        x5_im(mm+shiftB) = (x4_im(mm+shiftA) - x4_im(mm+shiftB)) * twiddle_im;
    end
end

% stage 6 -> N = 64, M = 1/2
twiddle_re = cos(-2 * pi * mm * 1/2);
twiddle_im = sin(-2 * pi * mm * 1/2);
