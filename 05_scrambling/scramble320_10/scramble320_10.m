...
fileID = fopen('scramble320_10_report.html', 'w');
doc_title = sprintf('Scrambling at 16 kHz, swapping 10 samples at a time');
doc_h1 = sprintf('Scrambling at 16 kHz, swapping 10 samples at a time');
doc_subtitle = sprintf('320 samples instead of 960; samples 1-80 and 241-320');
...
experiments = [
    ...
    4 5 7 6 3 1 2 0; % swap and scramble both
    7 5 4 6 1 3 0 2;
    7 6 5 4 2 1 0 3;
    5 4 6 7 3 1 2 0;
    5 6 4 7 1 0 3 2;
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        % 1-10, 11-20, ...
        a1 = experiments(ex, 1)*10+1;
        a2 = (experiments(ex, 1)+1)*10;
        b1 = experiments(ex, 2)*10+1;
        b2 = (experiments(ex, 2)+1)*10;
        c1 = experiments(ex, 3)*10+1;
        c2 = (experiments(ex, 3)+1)*10;
        d1 = experiments(ex, 4)*10+1;
        d2 = (experiments(ex, 4)+1)*10;
        e1 = experiments(ex, 5)*10+1;
        e2 = (experiments(ex, 5)+1)*10;
        f1 = experiments(ex, 6)*10+1;
        f2 = (experiments(ex, 6)+1)*10;
        g1 = experiments(ex, 7)*10+1;
        g2 = (experiments(ex, 7)+1)*10;
        h1 = experiments(ex, 8)*10+1;
        h2 = (experiments(ex, 8)+1)*10;

        temp1(1:80) =  [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2)];
        temp2(1:80) = [Zi(m,  a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2)];

        a1 = experiments(ex, 1)*10+241;
        a2 = (experiments(ex, 1)+1)*10+240;
        b1 = experiments(ex, 2)*10+241;
        b2 = (experiments(ex, 2)+1)*10+240;
        c1 = experiments(ex, 3)*10+241;
        c2 = (experiments(ex, 3)+1)*10+240;
        d1 = experiments(ex, 4)*10+241;
        d2 = (experiments(ex, 4)+1)*10+240;
        e1 = experiments(ex, 5)*10+241;
        e2 = (experiments(ex, 5)+1)*10+240;
        f1 = experiments(ex, 6)*10+241;
        f2 = (experiments(ex, 6)+1)*10+240;
        g1 = experiments(ex, 7)*10+241;
        g2 = (experiments(ex, 7)+1)*10+240;
        h1 = experiments(ex, 8)*10+241;
        h2 = (experiments(ex, 8)+1)*10+240;

        temp1(241:320) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2)];
        temp2(241:320) = [Zi(m,  a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2)];
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        temp1(1:80) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2)];
        temp2(1:80) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2)];
        ...
        temp1(241:320) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2)];
        temp2(241:320) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2)];
