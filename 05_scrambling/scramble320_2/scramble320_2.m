...
fileID = fopen('scramble320_2_report.html', 'w');
doc_title = sprintf('Scrambling at 16 kHz, swapping 2 samples at a time');
doc_h1 = sprintf('Scrambling at 16 kHz, swapping 2 samples at a time');
doc_subtitle = sprintf('320 samples instead of 960; samples 1-32 and 289-320');
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        % 1-2, 3-4, ...
        a1 = experiments(ex, 1)*2+1;
        a2 = (experiments(ex, 1)+1)*2;
        b1 = experiments(ex, 2)*2+1;
        b2 = (experiments(ex, 2)+1)*2;
        c1 = experiments(ex, 3)*2+1;
        c2 = (experiments(ex, 3)+1)*2;
        d1 = experiments(ex, 4)*2+1;
        d2 = (experiments(ex, 4)+1)*2;
        e1 = experiments(ex, 5)*2+1;
        e2 = (experiments(ex, 5)+1)*2;
        f1 = experiments(ex, 6)*2+1;
        f2 = (experiments(ex, 6)+1)*2;
        g1 = experiments(ex, 7)*2+1;
        g2 = (experiments(ex, 7)+1)*2;
        h1 = experiments(ex, 8)*2+1;
        h2 = (experiments(ex, 8)+1)*2;
        i1 = experiments(ex, 9)*2+1;
        i2 = (experiments(ex, 9)+1)*2;
        j1 = experiments(ex, 10)*2+1;
        j2 = (experiments(ex, 10)+1)*2;
        k1 = experiments(ex, 11)*2+1;
        k2 = (experiments(ex, 11)+1)*2;
        l1 = experiments(ex, 12)*2+1;
        l2 = (experiments(ex, 12)+1)*2;
        m1 = experiments(ex, 13)*2+1;
        m2 = (experiments(ex, 13)+1)*2;
        n1 = experiments(ex, 14)*2+1;
        n2 = (experiments(ex, 14)+1)*2;
        o1 = experiments(ex, 15)*2+1;
        o2 = (experiments(ex, 15)+1)*2;
        p1 = experiments(ex, 16)*2+1;
        p2 = (experiments(ex, 16)+1)*2;

        temp1(1:32) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2) Z(m, i1:i2) Z(m, j1:j2) Z(m, k1:k2) Z(m, l1:l2) Z(m, m1:m2) Z(m, n1:n2) Z(m, o1:o2) Z(m, p1:p2)];
        temp2(1:32) = [Zi(m, a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2) Zi(m, i1:i2) Zi(m, j1:j2) Zi(m, k1:k2) Zi(m, l1:l2) Zi(m, m1:m2) Zi(m, n1:n2) Zi(m, o1:o2) Zi(m, p1:p2)];

        a1 = experiments(ex, 1)*2+289;
        a2 = (experiments(ex, 1)+1)*2+288;
        b1 = experiments(ex, 2)*2+289;
        b2 = (experiments(ex, 2)+1)*2+288;
        c1 = experiments(ex, 3)*2+289;
        c2 = (experiments(ex, 3)+1)*2+288;
        d1 = experiments(ex, 4)*2+289;
        d2 = (experiments(ex, 4)+1)*2+288;
        e1 = experiments(ex, 5)*2+289;
        e2 = (experiments(ex, 5)+1)*2+288;
        f1 = experiments(ex, 6)*2+289;
        f2 = (experiments(ex, 6)+1)*2+288;
        g1 = experiments(ex, 7)*2+289;
        g2 = (experiments(ex, 7)+1)*2+288;
        h1 = experiments(ex, 8)*2+289;
        h2 = (experiments(ex, 8)+1)*2+288;
        i1 = experiments(ex, 9)*2+289;
        i2 = (experiments(ex, 9)+1)*2+288;
        j1 = experiments(ex, 10)*2+289;
        j2 = (experiments(ex, 10)+1)*2+288;
        k1 = experiments(ex, 11)*2+289;
        k2 = (experiments(ex, 11)+1)*2+288;
        l1 = experiments(ex, 12)*2+289;
        l2 = (experiments(ex, 12)+1)*2+288;
        m1 = experiments(ex, 13)*2+289;
        m2 = (experiments(ex, 13)+1)*2+288;
        n1 = experiments(ex, 14)*2+289;
        n2 =  (experiments(ex, 14)+1)*2+288;
        o1 = experiments(ex, 15)*2+289;
        o2 = (experiments(ex, 15)+1)*2+288;
        p1 = experiments(ex, 16)*2+289;
        p2 = (experiments(ex, 16)+1)*2+288;

        temp1(289:320) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2) Z(m, i1:i2) Z(m, j1:j2) Z(m, k1:k2) Z(m, l1:l2) Z(m, m1:m2) Z(m, n1:n2) Z(m, o1:o2) Z(m, p1:p2)];
        temp2(289:320) = [Zi(m, a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2) Zi(m, i1:i2) Zi(m, j1:j2) Zi(m, k1:k2) Zi(m, l1:l2) Zi(m, m1:m2) Zi(m, n1:n2) Zi(m, o1:o2) Zi(m, p1:p2)];
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        temp1(1:32) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2) X(m, i1:i2) X(m, j1:j2) X(m, k1:k2) X(m, l1:l2) X(m, m1:m2) X(m, n1:n2) X(m, o1:o2) X(m, p1:p2)];
        temp2(1:32) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2) Xi(m, i1:i2) Xi(m, j1:j2) Xi(m, k1:k2) Xi(m, l1:l2) Xi(m, m1:m2) Xi(m, n1:n2) Xi(m, o1:o2) Xi(m, p1:p2)];
        ...
        temp1(289:320) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2) X(m, i1:i2) X(m, j1:j2) X(m, k1:k2) X(m, l1:l2) X(m, m1:m2) X(m, n1:n2) X(m, o1:o2) X(m, p1:p2)];
        temp2(289:320) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2) Xi(m, i1:i2) Xi(m, j1:j2) Xi(m, k1:k2) Xi(m, l1:l2) Xi(m, m1:m2) Xi(m, n1:n2) Xi(m, o1:o2) Xi(m, p1:p2)];
