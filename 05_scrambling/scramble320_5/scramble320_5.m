...
fileID = fopen('scramble320_5_report.html', 'w');
doc_title = sprintf('Scrambling at 16 kHz, swapping 5 samples at a time');
doc_h1 = sprintf('Scrambling at 16 kHz, swapping 5 samples at a time');
doc_subtitle = sprintf('320 samples instead of 960; samples 1-40 and 281-320');
...
experiments = [
    ...
    4 5 6 7 0 1 2 3; % swap and scramble first half
    4 5 7 6 0 1 2 3;
    4 6 5 7 0 1 2 3;
    4 6 7 5 0 1 2 3;
    4 7 5 6 0 1 2 3;
    ...
    7 6 4 5 0 1 2 3;
    7 6 5 4 0 1 2 3;
    4 5 6 7 0 1 3 2; % swap and scramble latter half
    4 5 6 7 0 2 1 3;
    4 5 6 7 0 2 3 1;
    4 5 6 7 0 3 1 2;
    4 5 6 7 0 3 2 1;
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        % 1-5, 6-10, ...
        a1 = experiments(ex, 1)*5+1;
        a2 = (experiments(ex, 1)+1)*5;
        b1 = experiments(ex, 2)*5+1;
        b2 = (experiments(ex, 2)+1)*5;
        c1 = experiments(ex, 3)*5+1;
        c2 = (experiments(ex, 3)+1)*5;
        d1 = experiments(ex, 4)*5+1;
        d2 = (experiments(ex, 4)+1)*5;
        e1 = experiments(ex, 5)*5+1;
        e2 = (experiments(ex, 5)+1)*5;
        f1 = experiments(ex, 6)*5+1;
        f2 = (experiments(ex, 6)+1)*5;
        g1 = experiments(ex, 7)*5+1;
        g2 = (experiments(ex, 7)+1)*5;
        h1 = experiments(ex, 8)*5+1;
        h2 = (experiments(ex, 8)+1)*5;

        temp1(1:40) =  [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2)];
        temp2(1:40) = [Zi(m,  a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2)];

        a1 = experiments(ex, 1)*5+281;
        a2 = (experiments(ex, 1)+1)*5+280;
        b1 = experiments(ex, 2)*5+281;
        b2 = (experiments(ex, 2)+1)*5+280;
        c1 = experiments(ex, 3)*5+281;
        c2 = (experiments(ex, 3)+1)*5+280;
        d1 = experiments(ex, 4)*5+281;
        d2 = (experiments(ex, 4)+1)*5+280;
        e1 = experiments(ex, 5)*5+281;
        e2 = (experiments(ex, 5)+1)*5+280;
        f1 = experiments(ex, 6)*5+281;
        f2 = (experiments(ex, 6)+1)*5+280;
        g1 = experiments(ex, 7)*5+281;
        g2 = (experiments(ex, 7)+1)*5+280;
        h1 = experiments(ex, 8)*5+281;
        h2 = (experiments(ex, 8)+1)*5+280;

        temp1(281:320) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2)];
        temp2(281:320) = [Zi(m,  a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2)];
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        temp1(1:40) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2)];
        temp2(1:40) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2)];
        ...
        temp1(281:320) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2)];
        temp2(281:320) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2)];
