...
function detect_dtmf(dtmf_tone, dtmf_fs)
    ...
    lut = [
        ...
        sin(2 * pi * dtmf_freqs(8) * t)
        ];

    [
        max(xcorr(dtmf_tone, lut(1, 1:1600)))
        max(xcorr(dtmf_tone, lut(2, 1:1600)))
        max(xcorr(dtmf_tone, lut(3, 1:1600)))
        max(xcorr(dtmf_tone, lut(4, 1:1600)))
        max(xcorr(dtmf_tone, lut(5, 1:1600)))
        max(xcorr(dtmf_tone, lut(6, 1:1600)))
        max(xcorr(dtmf_tone, lut(7, 1:1600)))
        max(xcorr(dtmf_tone, lut(8, 1:1600)))
    ]
